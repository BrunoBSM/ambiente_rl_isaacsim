...
try:
    ...
    try:
        ...
        print("Warning: Could not import Transform utilities. Manual quaternion conversion will be used.")
        Transform = None
...
try:
    ...
    print("Warning: Could not import rotation utilities from isaacsim.core.utils.rotations")
...
class SimHelper:
    """
    Helper class para controle e interação com o robô GO2 no Isaac Sim.
    
    Esta classe gerencia a simulação, incluindo a criação do mundo, carregamento 
    do robô, aplicação de ações, coleta de observações e cálculo de recompensas
    para treinamento de reinforcement learning.
    
    Attributes:
        world (World): Instância do mundo de simulação do Isaac Sim
        robot (Articulation): Representação articulada do robô GO2
        stage_units (float): Fator de escala da simulação em metros
        last_base_pos (np.ndarray): Posição anterior da base para cálculo de recompensa
        joint_limits_lower (np.ndarray): Limites inferiores das juntas em radianos
        joint_limits_upper (np.ndarray): Limites superiores das juntas em radianos
        default_joint_positions (np.ndarray): Posições padrão das juntas (postura em pé)
        safety_margin (float): Margem de segurança para os limites das juntas
    """
    
    def __init__(self, safety_margin: float = 0.1):
        """
        Inicializa o helper da simulação com o robô GO2.
        
        Cria o mundo de simulação, carrega o modelo do robô GO2, configura 
        as articulações, obtém os limites das juntas e reseta o ambiente 
        para o estado inicial.
        
        Args:
            safety_margin (float): Margem de segurança para os limites das juntas.
                                 0.1 significa 10% de margem em cada extremo.
        
        Raises:
            Exception: Se houver erro ao carregar o modelo do robô ou inicializar o mundo
        """
        # Inicializa o mundo
        self.world = World(stage_units_in_meters=1.0)
        self.world.scene.add_default_ground_plane()

        # Carrega o modelo do GO2
        assets_root = get_assets_root_path()
        go2_usd_path = assets_root + "/Isaac/IsaacLab/Robots/Unitree/Go2/go2.usd"
        add_reference_to_stage(usd_path=go2_usd_path, prim_path="/World/Go2")

        # Inicializa a articulação do robô
        self.robot = Articulation(prim_paths_expr="/World/Go2", name="go2")

        # Obtém fator de escala da simulação
        self.stage_units = get_stage_units()

        # Salva posição anterior para cálculo de recompensa
        self.last_base_pos = np.zeros(3)
        
        # Margem de segurança
        self.safety_margin = safety_margin

        # Reseta o mundo no início para inicializar as estruturas
        self.world.reset()
        
        # Obtém os limites das juntas após o reset
        self._initialize_joint_limits()

    def _initialize_joint_limits(self):
        """
        Inicializa os limites das juntas do robô GO2 com margens de segurança.
        
        Obtém os limites físicos das juntas do modelo USD e aplica margens 
        de segurança para evitar movimentos extremos que possam causar 
        comportamentos instáveis.
        """
        print("[INFO] Inicializando limites das juntas...")
        
        # Primeiro, tenta obter os limites reais do modelo USD
        try:
            dof_limits = self.robot.get_dof_limits()
            print(f"[DEBUG] DOF limits obtidos: shape = {dof_limits.shape if dof_limits is not None else None}")
            
            if dof_limits is not None and len(dof_limits.shape) >= 2:
                # dof_limits shape: [num_robots, num_joints, 2] onde 2 = [lower, upper]
                self.joint_limits_lower = dof_limits[0, :, 0]  # Limites inferiores
                self.joint_limits_upper = dof_limits[0, :, 1]  # Limites superiores
                
                print(f"[SUCCESS] Limites reais obtidos do modelo USD:")
                print(f"  Lower limits: {self.joint_limits_lower}")
                print(f"  Upper limits: {self.joint_limits_upper}")
                
                # Verifica se os limites são válidos
                if self._validate_joint_limits():
                    print("[SUCCESS] Limites validados com sucesso!")
                else:
                    print("[WARNING] Limites inválidos, usando valores padrão...")
                    self._set_default_joint_limits()
            else:
                print("[WARNING] Não foi possível obter limites do USD, usando valores padrão...")
                self._set_default_joint_limits()
                
        except Exception as e:
            print(f"[ERROR] Erro ao obter limites das juntas: {e}")
            print("[INFO] Usando limites padrão como fallback...")
            self._set_default_joint_limits()
        
        # Aplica margem de segurança
        self._apply_safety_margins()
        
        # Define posições padrão baseadas nos limites reais
        self._set_default_positions()
        
        # Imprime resumo final
        self._print_joint_limits_summary()

    def _validate_joint_limits(self) -> bool:
        """
        Valida se os limites das juntas obtidos são razoáveis.
        
        Returns:
            bool: True se os limites são válidos, False caso contrário
        """
        # Verifica se temos exatamente 12 juntas
        if len(self.joint_limits_lower) != 12 or len(self.joint_limits_upper) != 12:
            print(f"[ERROR] Número incorreto de juntas: {len(self.joint_limits_lower)}")
            return False
        
        # Verifica se todos os limites são numéricos válidos
        if (not np.all(np.isfinite(self.joint_limits_lower)) or 
            not np.all(np.isfinite(self.joint_limits_upper))):
            print("[ERROR] Limites contêm valores inválidos (inf ou nan)")
            return False
        
        # Verifica se lower < upper para todas as juntas
        if not np.all(self.joint_limits_lower < self.joint_limits_upper):
            print("[ERROR] Limites inferior >= superior em algumas juntas")
            return False
        
        # Verifica se os ranges são razoáveis (não muito pequenos nem muito grandes)
        joint_ranges = self.joint_limits_upper - self.joint_limits_lower
        if np.any(joint_ranges < 0.1) or np.any(joint_ranges > 10.0):
            print(f"[WARNING] Ranges suspeitos nas juntas: {joint_ranges}")
    ...
    def _set_default_joint_limits(self):
        """
        Define limites padrão conservadores baseados em especificações típicas 
        de robôs quadrúpedes Unitree.
        
        Note:
            Estes valores são baseados em especificações gerais de robôs quadrúpedes
            e devem ser ajustados conforme as especificações reais do GO2.
        """
        print("[INFO] Usando limites padrão conservadores...")
        
        # Limites conservadores para quadrúpedes Unitree (em radianos)
        # Padrão típico: Hip ±45°, Thigh -45° a +90°, Calf -160° a -30°
        # Ordem: [FR_hip, FR_thigh, FR_calf, FL_hip, FL_thigh, FL_calf, 
        #         RR_hip, RR_thigh, RR_calf, RL_hip, RL_thigh, RL_calf]
        
        hip_limit = np.radians(45)      # ±45 graus para hip
        thigh_lower = np.radians(-45)   # -45 graus para thigh
        thigh_upper = np.radians(90)    # +90 graus para thigh  
        calf_lower = np.radians(-160)   # -160 graus para calf
        calf_upper = np.radians(-30)    # -30 graus para calf
        
        self.joint_limits_lower = np.array([
            -hip_limit, thigh_lower, calf_lower,  # Front Right
            -hip_limit, thigh_lower, calf_lower,  # Front Left
            -hip_limit, thigh_lower, calf_lower,  # Rear Right  
            -hip_limit, thigh_lower, calf_lower   # Rear Left
        ])
        
        self.joint_limits_upper = np.array([
            hip_limit, thigh_upper, calf_upper,   # Front Right
            hip_limit, thigh_upper, calf_upper,   # Front Left
            hip_limit, thigh_upper, calf_upper,   # Rear Right
            hip_limit, thigh_upper, calf_upper    # Rear Left
        ])
        
        print("[INFO] Limites padrão definidos (valores conservadores)")

    def _apply_safety_margins(self):
        """
        Aplica margens de segurança aos limites das juntas.
        """
        # Aplica margem de segurança
        joint_range = self.joint_limits_upper - self.joint_limits_lower
        margin = self.safety_margin * joint_range
        
        self.joint_limits_lower_safe = self.joint_limits_lower + margin
        self.joint_limits_upper_safe = self.joint_limits_upper - margin
        
        print(f"[INFO] Margem de segurança de {self.safety_margin*100:.1f}% aplicada")

    def _set_default_positions(self):
        """
        Define posições padrão baseadas nos limites reais das juntas.
        """
        # Posição neutra: no meio do range seguro para cada junta
        self.default_joint_positions = (
            self.joint_limits_lower_safe + self.joint_limits_upper_safe
        ) / 2.0
        
        # Ajusta posições específicas para uma postura mais natural
        # Hip joints: neutros (0)
        self.default_joint_positions[0::3] = 0.0  # Todas as hip joints
        
        # Thigh joints: ligeiramente flexionadas para cima
        thigh_indices = [1, 4, 7, 10]  # FR, FL, RR, RL thigh
        for i in thigh_indices:
            if i < len(self.default_joint_positions):
                # Posição a 1/3 do range a partir do limite inferior
                thigh_range = self.joint_limits_upper_safe[i] - self.joint_limits_lower_safe[i]
                self.default_joint_positions[i] = self.joint_limits_lower_safe[i] + thigh_range * 0.33
        
        # Calf joints: flexionadas para baixo (posição estável)
        calf_indices = [2, 5, 8, 11]  # FR, FL, RR, RL calf
        for i in calf_indices:
            if i < len(self.default_joint_positions):
                # Posição a 2/3 do range a partir do limite superior (mais flexionada)
                calf_range = self.joint_limits_upper_safe[i] - self.joint_limits_lower_safe[i]
                self.default_joint_positions[i] = self.joint_limits_upper_safe[i] - calf_range * 0.33

    def _print_joint_limits_summary(self):
        """
        Imprime um resumo detalhado dos limites das juntas.
        """
        joint_names = [
        ...
        print("\n" + "="*80)
        print("RESUMO DOS LIMITES DAS JUNTAS DO ROBÔ GO2")
        print("="*80)
        
        print(f"{'Junta':<12} {'Lower':<8} {'Upper':<8} {'Safe_Low':<8} {'Safe_Up':<8} {'Range':<8} {'Default':<8}")
        print("-"*80)
        
        for i, name in enumerate(joint_names):
            if i < len(self.joint_limits_lower):
                lower = self.joint_limits_lower[i]
                upper = self.joint_limits_upper[i] 
                safe_low = self.joint_limits_lower_safe[i]
                safe_up = self.joint_limits_upper_safe[i]
                range_val = upper - lower
                default = self.default_joint_positions[i]
                
                print(f"{name:<12} {lower:<8.3f} {upper:<8.3f} {safe_low:<8.3f} {safe_up:<8.3f} {range_val:<8.3f} {default:<8.3f}")
        
        print("-"*80)
        print(f"Margem de segurança: {self.safety_margin*100:.1f}%")
        print(f"Total de juntas: {len(joint_names)}")
        print("="*80 + "\n")

    def inspect_current_joint_positions(self) -> dict:
        """
        Inspeciona as posições atuais das juntas e verifica violações.
        
        Returns:
            dict: Informações detalhadas sobre o estado atual das juntas
        """
        joint_pos = self.robot.get_joint_positions().flatten()
        joint_names = [
            "FR_hip", "FR_thigh", "FR_calf",
            "FL_hip", "FL_thigh", "FL_calf", 
            "RR_hip", "RR_thigh", "RR_calf",
        ...
        for i, (name, pos) in enumerate(zip(joint_names, joint_pos)):
            detail = {
                ...
                "lower_safe": self.joint_limits_lower_safe[i],
                "upper_safe": self.joint_limits_upper_safe[i],
                "violates_hard": False,
                "violates_safe": False
            }
            
            # Verifica violações de limites duros
            if pos < self.joint_limits_lower[i] or pos > self.joint_limits_upper[i]:
                detail["violates_hard"] = True
                inspection["violations"]["hard_limits"].append(name)
                inspection["violations"]["count_hard"] += 1
            
            # Verifica violações de limites seguros
            if pos < self.joint_limits_lower_safe[i] or pos > self.joint_limits_upper_safe[i]:
                detail["violates_safe"] = True
                inspection["violations"]["safe_limits"].append(name)
                inspection["violations"]["count_safe"] += 1
            
            inspection["joint_details"].append(detail)
        
        return inspection

    def apply_action(self, action: np.ndarray):
        """
        Aplica ações de controle nas juntas do robô com limitações de segurança.
        
        Args:
            action (np.ndarray): Array com os valores de ação normalizadas entre -1 e 1.
                               Deve ter shape (n_joints,) onde n_joints é o número
                               de juntas controláveis do robô (tipicamente 12 para GO2).
                               
        Note:
            As ações são primeiro desnormalizadas para os limites seguros das juntas,
            depois limitadas (clipped) para garantir que não ultrapassem os limites
            físicos, e finalmente aplicadas como posições de junta.
        """
        # Converte ações normalizadas [-1, 1] para posições de junta reais
        action_clipped = np.clip(action, -1.0, 1.0)
        
        # Desnormaliza: mapeia [-1, 1] para [joint_limits_lower_safe, joint_limits_upper_safe]
        joint_positions = self._denormalize_action(action_clipped)
        
        # Aplica clipping adicional para garantir segurança absoluta
        joint_positions_safe = np.clip(
            joint_positions, 
            self.joint_limits_lower_safe, 
            self.joint_limits_upper_safe
        )
        
        # Aplica as posições no robô
        self.robot.set_joint_positions([joint_positions_safe])
    ...
    def _denormalize_action(self, normalized_action: np.ndarray) -> np.ndarray:
        ...
        Args:
            normalized_action (np.ndarray): Ações normalizadas entre -1 e 1
            
        Returns:
            np.ndarray: Posições de junta em radianos dentro dos limites seguros
        """
        # Mapeia [-1, 1] para [lower_safe, upper_safe]
        joint_range_safe = self.joint_limits_upper_safe - self.joint_limits_lower_safe
        joint_positions = (
            self.joint_limits_lower_safe + 
            (normalized_action + 1.0) * 0.5 * joint_range_safe
        )
        return joint_positions

    def apply_action_relative(self, action: np.ndarray, scale: float = 0.1):
        """
        Aplica ações de controle como mudanças relativas das posições atuais.
        
        Esta é uma alternativa mais suave ao controle absoluto, onde as ações
        representam pequenas mudanças nas posições das juntas.
        
        Args:
            action (np.ndarray): Array com mudanças relativas normalizadas [-1, 1]
            scale (float): Fator de escala para as mudanças (menor = mais suave)
        """
        # Obtém posições atuais
        current_positions = self.robot.get_joint_positions().flatten()
        
        # Calcula mudanças relativas
        action_clipped = np.clip(action, -1.0, 1.0)
        joint_range_safe = self.joint_limits_upper_safe - self.joint_limits_lower_safe
        position_changes = action_clipped * scale * joint_range_safe
        
        # Aplica mudanças
        new_positions = current_positions + position_changes
        
        # Limita às zonas seguras
        new_positions_safe = np.clip(
            new_positions,
            self.joint_limits_lower_safe,
            self.joint_limits_upper_safe
        )
        
        self.robot.set_joint_positions([new_positions_safe])

    def step_simulation(self, render: bool = True):
        """
        Avança um passo da simulação física.
        
        Args:
            render (bool, optional): Se True, renderiza a simulação visualmente.
                                   Se False, executa apenas a física sem renderização.
                                   Default é True.
                                   
        Note:
            Cada passo representa um frame de simulação física, tipicamente
            1/60 segundos na configuração padrão do Isaac Sim.
        """
        self.world.step(render=render)

    def reset(self):
        """
        Reseta o mundo de simulação e o robô para o estado inicial.
        
        Reposiciona o robô na posição inicial (0, 0, 1.0 metros de altura),
        reseta a física da simulação, coloca as juntas em posições padrão
        seguras e atualiza a posição de referência para cálculo de recompensas.
        
        Note:
            Esta função deve ser chamada no início de cada episódio de treinamento.
        """
        self.world.reset()
        
        # Reposiciona o robô
        self.robot.set_world_poses(
            positions=np.array([[0.0, 0.0, 1.0]]) / self.stage_units
        )
        
        # Coloca as juntas em posições padrão seguras
        self.robot.set_joint_positions([self.default_joint_positions])
        
        # Atualiza referência para recompensa
        self.last_base_pos = self.robot.get_world_poses()[0].flatten()

    def get_observation(self) -> np.ndarray:
        """
        Coleta e retorna as observações completas do estado atual do robô.
        
        Returns:
            np.ndarray: Vetor de observações concatenado contendo:
                       - Posições das juntas (n_joints elementos)
                       - Velocidades das juntas (n_joints elementos) 
                       - Posição da base no mundo (3 elementos: x, y, z)
                       - Velocidade da base (6 elementos: vel linear + vel angular)
                       
        Note:
            As posições das juntas são referentes aos ângulos das articulações,
            não às posições cartesianas dos links.
            As velocidades das juntas são as velocidades angulares das articulações.
        """

        joint_pos = self.robot.get_joint_positions().flatten()
        joint_vel = self.robot.get_joint_velocities().flatten()
        base_pos = self.robot.get_world_poses()[0].flatten()
        base_vel = self.robot.get_velocities()[0].flatten()

        obs = np.concatenate([joint_pos, joint_vel, base_pos, base_vel])
        return obs

    def calculate_reward(self) -> float:
        """
        Calcula a recompensa baseada no desempenho do robô.
        
        A função de recompensa considera:
        - Progresso no eixo X (movimento para frente)
        - Penalidades por queda (altura < 0.2m)
        - Penalidades por movimentos verticais excessivos
        - Penalidades por violação dos limites das juntas
        
        Returns:
            float: Valor da recompensa para o passo atual.
                  Valores positivos indicam bom desempenho,
                  valores negativos indicam comportamentos indesejados.
                  
        Note:
            A recompensa é calculada baseada na diferença de posição desde
            o último passo, incentivando movimento contínuo para frente.
        """
        base_pos = self.robot.get_world_poses()[0].flatten()
        base_vel = self.robot.get_velocities()[0].flatten()
        joint_pos = self.robot.get_joint_positions().flatten()

        # Recompensa por progresso para frente
        reward = base_pos[0] - self.last_base_pos[0]
        ...
        # Penalidade por excesso de velocidade vertical
        if abs(base_vel[2]) > 0.1:
            reward -= 10
            
        # Penalidade por violação dos limites das juntas (segurança adicional)
        joint_violations = np.sum(
            (joint_pos < self.joint_limits_lower_safe) | 
            (joint_pos > self.joint_limits_upper_safe)
        )
    ...
    def check_termination(self) -> bool:
        """
        Verifica se o episódio deve ser terminado.
        
        Returns:
            bool: True se o episódio deve terminar, False caso contrário.
                 O episódio termina quando:
                 - O robô cai (altura < 0.2m)
                 - Há violação grave dos limites das juntas
                 
        Note:
            Esta função é chamada a cada passo para determinar se o episódio
            de treinamento deve ser encerrado devido a falha ou condição de parada.
        """
        base_pos = self.robot.get_world_poses()[0].flatten()
        joint_pos = self.robot.get_joint_positions().flatten()
        
        # Termina se o robô caiu
        if base_pos[2] < 0.2:
            return True
            
        # Termina se há muitas violações dos limites das juntas
        joint_violations = np.sum(
            (joint_pos < self.joint_limits_lower) | 
            (joint_pos > self.joint_limits_upper)
        )
        if joint_violations > 3:  # Permite algumas violações menores
            print(f"[WARNING] Episódio terminado por violação dos limites das juntas: {joint_violations}")
            return True
    ...
    def get_joint_info(self) -> dict:
        """
        Retorna informações sobre os limites e estado atual das juntas.
        
        Returns:
            dict: Dicionário com informações das juntas incluindo limites,
                 posições atuais e nomes das juntas.
        """
        joint_pos = self.robot.get_joint_positions().flatten()
        ...
        return {
            ...
            "lower_limits_safe": self.joint_limits_lower_safe,
            "upper_limits_safe": self.joint_limits_upper_safe,
            "default_positions": self.default_joint_positions
        }

    def close(self):
        """
        Fecha e limpa a simulação.
        
        Encerra o SimulationApp e libera os recursos da simulação.
        Esta função deve ser chamada ao final do treinamento ou quando
        o ambiente não for mais necessário.
        
        Warning:
            Após chamar esta função, o objeto SimHelper não deve mais ser usado.
        """
